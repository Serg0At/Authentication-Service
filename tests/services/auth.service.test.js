...
jest.unstable_mockModule('../../src/config/variables.config.js', () => ({
  default: {
    ...
    REDIS: { TTL: { USER_SESSIONS: 604800, RESET_CODE: 900, REFRESH_TOKEN: 2592000 } },
...
describe('AuthService.register', () => {
  const registerData = { email: 'new@test.com', username: 'newuser', password: 'password123', userAgent: 'Mozilla/5.0' };
  ...
  it('should publish user.registered event', async () => {
    ...
    expect(mockPublishAuthEvent).toHaveBeenCalledWith(
      'user.registered',
      expect.objectContaining({ user_id: 2, email: 'new@test.com', username: 'newuser' }),
    );
  });

  it('should generate access and refresh tokens', async () => {
    await AuthService.register(registerData);
    expect(mockJwtUtil.generateAccessToken).toHaveBeenCalled();
...
describe('AuthService.changePassword', () => {
  ...
  it('should change password successfully', async () => {
    const result = await AuthService.changePassword({ oldPass: 'old', newPass: 'newpass123', accessToken: 'token' });
  ...
  it('should throw UnauthorizedError if access token invalid', async () => {
    ...
    await expect(AuthService.changePassword({ oldPass: 'old', newPass: 'new', accessToken: 'bad' })).rejects.toMatchObject({
      name: 'UnauthorizedError',
    });
  });

  it('should throw InvalidPasswordError if old password wrong', async () => {
    mockCryptoUtil.comparePassword.mockResolvedValue(false);

    await expect(AuthService.changePassword({ oldPass: 'wrong', newPass: 'new', accessToken: 'token' })).rejects.toMatchObject({
      name: 'InvalidPasswordError',
    });
  });

  it('should hash new password before storing', async () => {
    await AuthService.changePassword({ oldPass: 'old', newPass: 'newpass123', accessToken: 'token' });
    expect(mockCryptoUtil.hashPassword).toHaveBeenCalledWith('newpass123');
  });
...
describe('AuthService.verify2FA', () => {
  ...
  it('should generate access token with acr: 2fa claim', async () => {
    await AuthService.verify2FA(verifyData);
